...
$jobs = new myJAM_Jobs();
$ret_array = $jobs->GetDistCoresLOG2($args);
$chart = new graph();
$bar = new bar_outline(50, '#3333CC', '#1010A0');
$bar->key('# Cores', 12);
$vXLab = array();
$np_lab = 1;
for($nbcores = 0; $nbcores <= $ret_array['MaxCoresLOG2']; $nbcores++)
{
  ...
  {
    $bar->add_link($ret_array['data'][$nbcores], 'javascript:pop_CoreLOG2Info(\''.$nbcores.'\')');
  }
  else
  {
