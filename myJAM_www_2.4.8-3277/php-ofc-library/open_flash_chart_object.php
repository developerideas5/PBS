...
function open_flash_chart_object( $width, $height, $url, $use_swfobject=true, $base='', $id='chart' )
{
    //
    // stream the HTML into the page
    //
    echo _ofc( $width, $height, $url, $use_swfobject, $base, $id );
}
function _ofc( $width, $height, $url, $use_swfobject, $base, $id )
{
    //
...
//$out[] = '</object>';
 //$out[]='<object type="application/x-shockwave-flash" data="'. $base .'open-flash-chart.swf?width='. $width .'&amp;height='. $height . '&amp;data='. $url .'" width="'. $width .'" height="'. $height .'" id="'.$id.'"/>';
 $out[]='<object type="application/x-shockwave-flash" data="'. $base .'open-flash-chart.swf?width='. $width .'&amp;height='. $height . '&amp;data='. $url .'" width="'. $width .'" height="'. $height .'" id="'.$id.'">';
 $out[]='<param name="'.$id.'" value="'. $base .'open-flash-chart.swf?width='. $width .'&amp;height='. $height . '&amp;data='. $url .'" />';
 $out[]='<param name="wmode" value="opaque"/>';# FIXME: check for sideeffects, wmode value changed to value="opaque", from "transparent"
 $out[]='</object>';
    if ( $use_swfobject ) {
 $out[] = '</noscript>';
    }
