...
$jobs = new myJAM_Jobs;
$ret_array = $jobs->GetDistUser($args);
...
for($i=0; $i<$SelectTop; $i++)
{
  if($admin)
  {
    $vlinks[$i] = 'javascript:pop_UserInfo(\''.$ret_array['UserNames'][$i].'\')';
  }
  else
  {
    $vlinks[$i] = '';
...
$title = 'Top '.$SelectTop.' Users'.$title;
if ($args['type'] == 'PIE')
{
  $pie = new graph();
  ...
  $pie->pie_values(array_slice($ret_array['data'], 0, $SelectTop),
                   array_slice($ret_array['UserNames'], 0, $SelectTop),
...
elseif ($args['type'] == 'BARS')
{
  ...
  $chart->data_sets[] = $bar;
  $chart->set_x_labels(array_slice($ret_array['UserNames'], 0, $SelectTop));
